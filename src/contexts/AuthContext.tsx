...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // Carrega cliente do localStorage ao montar e verifica se ainda existe
  useEffect(() => {
    const carregarEVerificar = async () => {
      ...
      if (clienteSalvo) {
        try {
          ...
          const existe = await verificarClienteExiste(clienteData.id)

          if (existe) {
            setCliente(clienteData)
          } else {
            // Cliente foi deletado do banco - fazer logout
            console.log('Cliente não existe mais no banco - fazendo logout automático')
